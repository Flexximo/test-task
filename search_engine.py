    ...
    for t in tickets:
        ...
        try:
            ...
            logging.basicConfig(filename='logger.log', 
                                ...
                                datefmt="%d/m%/%Y", 
                                level=logging.INFO)
            logging.info(f"Unable to establish connection with: {url}, response: {error.response.content}")
            sys.stderr.write(f"Error occured, watch log file for errors")    
            ...
            logging.basicConfig(filename='logger.log', 
                                ...
                                datefmt="%d/m%/%Y", 
                                level=logging.INFO)
            logging.info(f"Unable to handle request: {url}, response: {error.response.content}")
            sys.stderr.write(f"Error occured, log from source: {error.response.content}")       
        else:
...
def searchEngine(fromTo, toFrom, a, i, c, date_departure, date_arrival=None):
    ...
    if not date_arrival:
        ...
        if a == 1 and i == 1 and c == 1:                          
           respond = sendData(date_departure, is_one_way, 0, fromTo_)    
        elif a == 1 and i == 1 and c == 0:
           respond = sendData(date_departure, is_one_way, 1, fromTo_)    
        elif a == 1 and i == 0 and c == 1:
           respond = sendData(date_departure, is_one_way, 2, fromTo_)  
        elif a == 1 and i == 0 and c == 0:
           respond = sendData(date_departure, is_one_way, 3, fromTo_)    
        elif a == 2 and i == 1 and c == 1:
           respond = sendData(date_departure, is_one_way, 4, fromTo_)    
        elif a == 2 and i == 1 and c == 0:
           respond = sendData(date_departure, is_one_way, 5, fromTo_)
        elif a == 2 and i == 0 and c == 1:
           respond = sendData(date_departure, is_one_way, 6, fromTo_)
        elif a == 2 and i == 0 and c == 0:
           respond = sendData(date_departure, is_one_way, 7, fromTo_)
        print("I am here")
        isValid_ = isValid(respond, a, i, c)
        message = f"Found for {fromTo_[0]}-{fromTo_[0]}. Price {isValid_[0][0]} USD"
        print(message)    
        ...
        if a == 1 and i == 1 and c == 1:                          
           ...
           respond = sendData(date_departure, is_one_way, 1, fromTo_, toFrom_, date_arrival=date_arrival)    
        elif a == 1 and i == 0 and c == 1:
           respond = sendData(date_departure, is_one_way, 2, fromTo_, toFrom_, date_arrival=date_arrival)  
        elif a == 1 and i == 0 and c == 0:
           respond = sendData(date_departure, is_one_way, 3, fromTo_, toFrom_, date_arrival=date_arrival)    
        elif a == 2 and i == 1 and c == 1:
           respond = sendData(date_departure, is_one_way, 4, fromTo_, toFrom_, date_arrival=date_arrival)    
        elif a == 2 and i == 1 and c == 0:
           respond = sendData(date_departure, is_one_way, 5, fromTo_, toFrom_, date_arrival=date_arrival)
        elif a == 2 and i == 0 and c == 1:
           respond = sendData(date_departure, is_one_way, 6, fromTo_, toFrom_, date_arrival=date_arrival)
        elif a == 2 and i == 0 and c == 0:
           respond = sendData(date_departure, is_one_way, 7, fromTo_, toFrom_, date_arrival=date_arrival)
        isValid_ = isValid(respond, a, i, c)
...
def searchDaily(cache=cache, start=datetime.today(), delta=timedelta(days=31), directions=directions, tickets=tickets, curr="USD", a=None, i=None, c=None):
    ...
    for d in directions:        
        ...
        try :
        ...
        #if connection or request fails create and write to a log file.
        except requests.exceptions.ConnectionError:
            logging.basicConfig(filename='logger.log', 
                                filemode="a+", 
                                format="%(asctime)s - %(message)s", 
            ...
            #Basic logic for illing the cache with all tickets data for amount of passengers
            ...
            if d == ("ALA", "TSE"):                  
                ...
                elif a == 1 and c == 1 and i == 0:
                    for n in tickets[d]:
                        cache["data"][d][1]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
                elif a == 1 and c == 0 and i == 1:
                    for n in tickets[d]:
                        cache["data"][d][2]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
                elif a == 1 and c == 0 and i == 0:
                    for n in tickets[d]:
                        cache["data"][d][3]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
                elif a == 2 and c == 1 and i == 1:
                    for n in tickets[d]:
                        cache["data"][d][4]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
                elif a == 2 and c == 1 and i == 0:
                    for n in tickets[d]:
                        cache["data"][d][5]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
                elif a == 2 and c == 0 and i == 1:
                    for n in tickets[d]:
                        cache["data"][d][6]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
                elif a == 2 and c == 0 and i == 0:
                    for n in tickets[d]: 
                        cache["data"][d][7]["tickets"].append([n["price"], time.strftime('%d/%m/%Y', time.localtime(n["dTime"])), time.strftime('%d/%m/%Y', time.localtime(n["aTime"])), n["booking_token"],])
...
def filteredData(cache=tickets, handler=searchDaily):
    ...
    processD = Thread(target=handler, kwargs={"a": 1, "i": 0, "c": 0}).start()
    processE = Thread(target=handler, kwargs={"a": 2, "i": 1, "c": 1}).start()
    processF = Thread(target=handler, kwargs={"a": 2, "i": 1, "c": 0}).start()
    processG = Thread(target=handler, kwargs={"a": 2, "i": 0, "c": 1}).start()
    processH = Thread(target=handler, kwargs={"a": 2, "i": 0, "c": 0}).start()
